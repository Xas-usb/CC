...
function [sys,x0,str,ts] = mdlInitializeSizes
    ...
    sizes.NumInputs      = 15; % 期望轨迹6 + 误差2 + v_bar + 约束6
...
function sys = mdlOutputs(t,x,u_in)
    ...
    %% --- 5. 安全除法保护：防止 a1/a2=0 导致 c/a 或 d/a 发散 ---
    a1_safe = sign(a1) * max(abs(a1), 1e-10);
    a2_safe = sign(a2) * max(abs(a2), 1e-10);

    %% --- 6. Lyapunov 函数 V (式2-30, 分母加极小量防除零) ---
    Vu = 0.5 * (rho_u^2 / (1 - rho_u^2 + 1e-30));
    Vr = 0.5 * (rho_r^2 / (1 - rho_r^2 + 1e-30));

    %% --- 7. 虚拟控制律 alpha (式2-42) ---
    term_Vu = (pi / (gamma * Tp)) * (Vu^(1 - 0.5*gamma) + Vu^(1 + 0.5*gamma));
    term_Vr = (pi / (gamma * Tp)) * (Vr^(1 - 0.5*gamma) + Vr^(1 + 0.5*gamma));

    % 保护：分母 cos(phi_e) 接近 0 时截断
    cos_phi_e_safe = sign(cos(phi_e)) * max(abs(cos(phi_e)), 0.01);

    alpha_u = (xd_dot * cos(phi_d) + yd_dot * sin(phi_d) - v_bar * sin(phi_e) + ...
               (c1 / a1_safe) * term_Vu - (d1 / a1_safe) + K1 * E) / cos_phi_e_safe;

    alpha_r = phid_dot + (c2 / a2_safe) * term_Vr - (d2 / a2_safe) + K2 * phi_e;
