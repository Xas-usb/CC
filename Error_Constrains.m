...
function sys = mdlOutputs(t,x,u_in)
    ...
    % t >= Tf 时保持在 th_inf，避免 (Tf-t) 除零
    if t < Tf - 1e-8  % 留安全余量，避免 Tf-t → 0 的数值问题
        exp_term = exp(1 - Tf/(Tf - t));
        theta1 = (th10 - th1_inf) * exp_term + th1_inf;
        theta2 = (th20 - th2_inf) * exp_term + th2_inf;

        % theta 导数 (需要传递到虚拟控制律)
        theta1_dot = -((th10 - th1_inf) * Tf / ((Tf - t)^2 + 1e-10)) * exp_term;
        theta2_dot = -((th20 - th2_inf) * Tf / ((Tf - t)^2 + 1e-10)) * exp_term;
    else
        % t >= Tf: 保持在稳态值
        theta1 = th1_inf;
        theta2 = th2_inf;
    ...
    % 安全处理：E 或 phi_e 极大时，sigma 饱和到 ±k
    sigma1 = (k1 * E) / sqrt(E^2 + l1 + 1e-30);  % 加极小量防除零
    sigma2 = (k2 * phi_e) / sqrt(phi_e^2 + l2 + 1e-30);
